        ...
        <form className={style.regFormContainer} onSubmit={handleSubmit}>
            ...
            {showCreateAccountPage ?
                <>
                    ...
                    <TextField  sx={{width:"85%"}} id="outlined-basic" label="Name" variant="outlined" value={name} onChange={(e) => setName(e.target.value)} />
                    {errors.name && <span className={style.error}>{errors.name}</span>}<br />
                    {showEmailBox ? <>
                        <TextField  sx={{width:"85%"}} id="outlined-basic" label="Email" variant="outlined" value={email} onChange={(e) => setEmail(e.target.value)} />
                        <span className={style.userContactChoice} onClick={toggleEmailPhone}>Use Phone instead</span>
                        {errors.email && <span className={style.error}>{errors.email}</span>}

                    </> :
                        <>   <TextField  sx={{width:"85%"}}  id="outlined-basic" label="Phone" variant="outlined" value={phone} onChange={(e) => setPhone(e.target.value)} />
                    ...
                    <label className={style.dobHeading}><strong>Date of Birth</strong></label>
                    <p className={style.dobInfo}>This will not be shown publicly. Confirm your own age, even if this account is for a business, a pet, or something else.</p>
